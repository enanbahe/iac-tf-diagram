...
locals {
  ...
  s3_bucket_name			= "ace-blueprint-usw2-test-terraform-state" # S3 bucket where Terraform state will be stored. For this example is also used to store the lambda authorizer source code.
	s3_bucket_key_base  = "infrastructure/modules/integration/api/lambda-authorizer/examples/terragrunt/iam-policy-store/id-token" # Base Path for S3 to store Terraform state and lambda authorizer source code.
  ...
  dynamodb_table_name = "terraform-locks" # -------------------------- Fixed value for DynamoDB table for storing the status of the Terraform executions for ACE Examples
	...
	# Input variables
  namespace     = "${replace(local.namespace_base, "_", "-")}-iam-id-token" # Normally a long namespace is not required but there are many examples within the same account.
...
inputs = {
  ...
  app_role_map_store = "iam" # IAM Policies to map app roles will be stored in IAM which is more secure but increases latency in validations (greater than 400ms)
  app_role_map = {
    ACE_ADMIN = [{ # ACE_ADMIN must be a registered app role in Azure AD for this application
      ...
      resource_path = ["iam-based/*"] # All resources under iam-based path in the API Gateway will be allowed for this app role
    }]
    ACE_RW = [{ # ACE_RW must be a registered app role in Azure AD for this application
      ...
      resource_path = ["iam-based/admin", "iam-based/admin/*"] # All resources under iam-based/admin path in the API Gateway will be denied for this app role
    },{
      effect        = "Allow"
      http_method   = ["*"] # All methods (GET, PUT, POST, etc.) will be allowed for this app role
      resource_path = ["iam-based/caas", "iam-based/caas/*"] # All resources under iam-based/caas path in the API Gateway will be allowed for this app role
    }]
