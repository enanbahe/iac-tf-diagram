...
locals {
  ...
  s3_bucket_name			= "ace-blueprint-usw2-test-terraform-state" # S3 bucket where Terraform state will be stored. For this example is also used to store the lambda authorizer source code.
	s3_bucket_key_base  = "infrastructure/modules/integration/api/lambda-authorizer/examples/terragrunt/dydb-policy-store/access-token" # Base Path for S3 to store Terraform state and lambda authorizer source code.
  ...
  dynamodb_table_name = "terraform-locks" # -------------------------- Fixed value for DynamoDB table for storing the status of the Terraform executions for ACE Examples
	...
	# Input variables
  namespace     = "${replace(local.namespace_base, "_", "-")}-dydb-access-token" # Normally a long namespace is not required but there are many examples within the same account.
